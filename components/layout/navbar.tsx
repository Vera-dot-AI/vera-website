  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-[#0b0813]/85 backdrop-blur-md border-b border-white/10"
          : "bg-transparent"
      }`}
    >
      <nav
      ...
      >
        ...
        <Link href="/" className="flex items-center gap-3 group" aria-label="Vera home">
          <div className="w-9 h-9 relative flex-shrink-0 transition-transform duration-300 group-hover:scale-105">
            <Logo className="w-full h-full text-[#9945ff] drop-shadow-[0_0_8px_rgba(153,69,255,0.4)]" />
          </div>
          <span
            className="font-barlow text-2xl font-bold text-white tracking-wider uppercase"
            style={{ lineHeight: 1 }}
          >
            VERA
          </span>
        ...
        {/* Desktop: Right side - CTA and Login style button */}
        <div className="hidden md:flex items-center gap-4">
          ...
          <div className="relative group">
            {/* White glow effect behind navbar login button */}
            ...
            >
              <span className="tracking-wider text-black font-bold">Login</span>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-[#0c0814] border-t border-white/10 px-6 py-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
          ...
          <div className="border-t border-white/10 pt-4 mt-2 flex flex-col gap-3">
            <Link
              href="/#contact"
              className="btn-outline-premium text-center text-xs w-full py-3"
              onClick={() => setMenuOpen(false)}
            >
              Contact us
            </Link>
            <Link
              ...
              onClick={() => setMenuOpen(false)}
            >
              <span className="text-black font-semibold text-xs tracking-wider">Request Access</span>
